...
void loop() {
  ...
  for(int x=0; x < samples; x++) {
    ...
    SET(PORTF, 0); //direct port manipulate the chip enable pin for more consistent results
    sei();
    ...
    CLR(PORTF, 0) ;//direct port manipulate the chip enable pin
    TCCR0B = timercache; //restore timer0
    Serial.begin(serialBaud); //restore serial
    
    if(stat==2) { //max_rt was triggered, restart measurement
        goto restart;
      }
