...
# TODO: call cp /CMC/setups/ensc450/rgb2grayscale/syn_045/.synopsys_vss.setup .
# setup synth folders
#       mkdir work
#       mkdir logs
...
def auto_run():
    ...
    step = 1.0
    a_range = np.arange(start, end + step, step)
...
def parse_hspice(file):
    cells = ["INV 2C X1_layout", "INV 10C X1_layout", "NOR 2C X1_layout", "NOR 10C X1_layout", "NAND 2C X2_layout", "NAND 10C X2_layout"]
    measure_key = ["tpd_fall", "tpd_rise", "ttr_fall", "ttr_rise"]
    ...
    for item in cells:
        ...
        for i in range(0, len(section)):
            if "parameter" in section[i] and "warning" not in section[i]:
                data.append("\n" + section[i])
            for cell in cells:
                if cell.lower() in section[i]:
                    data.append(section[i])
            for key in measure_key:
                if key in section[i] and "meas_variable" not in section[i]:
                    data.append(section[i])
        ...
        for i in range(0, len(data)):
            if "parameter" in data[i]:
                ...
                if t_time == "1.000E-09":
                    ...
                    index_1 = "2ns"
                gates[gate + "_" + ds][index_1 + "," + index_2] = {}
                gates[gate + "_" + ds][index_1 + "," + index_2][measure_key[0]] = data[i + 2].split(" ")[5]
                gates[gate + "_" + ds][index_1 + "," + index_2][measure_key[1]] = data[i + 3].split(" ")[5]
                gates[gate + "_" + ds][index_1 + "," + index_2][measure_key[2]] = data[i + 4].split(" ")[5]
                gates[gate + "_" + ds][index_1 + "," + index_2][measure_key[3]] = data[i + 5].split(" ")[5]
            print(data[i])
...
if __name__ == "__main__":
    ...
    # combine_data("data")
    # parse_hspice("../layout_cir/hspice_layout.out")
