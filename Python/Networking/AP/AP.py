...
try:
    print(header + "AP Creator by TRIWAVE SYSTEMS\n")
except:
    None
...
def _run_cmd_write(cmd_args, s):
    ...
    p = subprocess.Popen(cmd_args, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, shell=False, universal_newlines=True)
    p.stdin.write(s)
    p.stdin.close()
    p.wait()
...
try:
    ...
    ap_iface = "wlx088005bad428"
    net_iface = "wlp4s0"

    print("[I] Wireless interface (for the AP): " + ap_iface)
    print("[I] Internet connected interface (for the AP): " + net_iface)
    ...
    ssid = "Akriveia"
    print("[I] Access Point SSID: " + ssid)
    while True:
        # channel = input("[?] Please enter the channel for the AP: ")
        channel = "1"
        print("[I] Access Point channel: " + channel)
        if channel.isdigit():
            break
        else:
    ...
    print("[I] Enable WPA2 encryption")
    if hostapd_wpa == "y":
        canBreak = False
        while not canBreak:
            # wpa_passphrase = input("[?] Please enter the WPA2 passphrase for the AP: ")
            wpa_passphrase = "1234567890"
            print("[I] Enable WPA2 encryption passphrase: " + wpa_passphrase)
