...
void stats_record_consumed(int factory_number, double delay_in_ms)
{
	...
	stats[factory_number].totalDelay = stats[factory_number].totalDelay + delay_in_ms;
...
{     
	...
	for (int i = 0; i < producers; i++)
	{
		...
		{
			printf("%8d%10d%10d%20.5f%20.5f%20.5f\n", stats[i].factoryID, stats[i].made, stats[i].eaten, stats[i].minDelay, stats[i].avgDelay, stats[i].maxDelay);
		}
	} 
}
